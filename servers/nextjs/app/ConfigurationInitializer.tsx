...
export function ConfigurationInitializer({ children }: { children: React.ReactNode }) {
  ...
  // Existing decks can be opened without an LLM provider. Requiring a valid
  // generation configuration here prevented direct presentation links from
  // rendering and redirected users to the setup page instead.
  const isPresentationRoute =
    route === "/presentation" || route?.startsWith("/presentation/");
  ...
  const fetchUserConfigState = async () => {
    ...
    if (canChangeKeys) {
      ...
      if (isValid) {
        ...
        route !== '/' &&
        !isPresentationRoute &&
        !(isSettingsRoute && llmConfig.LLM === 'codex')
      ) {
        router.push('/');
        setLoadingToFalseAfterNavigatingTo('/');
      } else {
