...
function getAppDataDirectory(appRoot: string): string {
  return (
    process.env.APP_DATA_DIRECTORY?.trim() || path.join(appRoot, "app_data")
  );
}
...
function bundledConverterPath(exportRoot: string): string {
  ...
  if (process.platform === "win32") {
    ...
    if (process.arch === "arm64") {
      return path.join(exportRoot, "py", "convert-win32-arm64.exe");
    }
  }
...
async function runBundledPresentationExportLocked(params: {
  ...
  const appDataDirectory = getAppDataDirectory(appRoot);

  await fs.access(converter);

  // `next dev` listens on 3000, whereas the production container has nginx
  // listening on port 80. An explicit NEXT_PUBLIC_URL still wins for Electron
