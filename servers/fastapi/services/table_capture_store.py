...
def _capture_path(token: str) -> str:
    ...
    if not safe_token:
        raise ValueError("empty table capture token")
    return os.path.join(_capture_directory(), f"{safe_token}.json")


def write_capture(token: str, presentation_id: str, tables: list[dict]) -> None:
    """Best-effort. Logs and returns on any failure rather than raising."""
    try:
        path = _capture_path(token)
    except ValueError:
        LOGGER.warning("table_capture_store: refusing to write capture for empty token")
        return

    payload = {"presentation_id": presentation_id, "tables": tables}
    ...
    try:
        ...
        LOGGER.exception(
            "table_capture_store: failed to write capture for token=%s", token
        )
        try:
            os.remove(tmp_path)
        except OSError:
...
def take_capture(token: str) -> Optional[dict[str, Any]]:
    ...
    try:
        ...
        LOGGER.exception(
            "table_capture_store: failed to read capture for token=%s", token
        )
        data = None
    finally:
        try:
...
def sweep_stale_captures(max_age_seconds: int = _STALE_MAX_AGE_SECONDS) -> None:
    ...
    try:
        ...
        LOGGER.exception("table_capture_store: failed to sweep stale captures")
