...
function unzipArchive(zipPath, destDir) {
  ...
  try {
    ...
    // Windows does not include the Unix `unzip` utility, but modern Windows
    // ships bsdtar, which can safely extract zip archives.
...
async function downloadAndInstallRuntime(tag) {
  ...
  if (fs.existsSync(zipPath)) {
    console.log(`[presentation-export] Using cached archive ${zipPath}`);
  } else {
