...
function resolveRuntimeAsset() {
  ...
  if (process.platform === "linux") {
    if (arch === "amd64" || arch === "x64") {
      return { assetName: "export-Linux-X64.zip", arch: "x64" };
    }
    if (arch === "arm64" || arch === "aarch64") {
      return { assetName: "export-Linux-ARM64.zip", arch: "arm64" };
    }
  }

  if (process.platform === "win32") {
    if (arch === "amd64" || arch === "x64") {
      return { assetName: "export-Windows-X64.zip", arch: "x64" };
    }
    if (arch === "arm64" || arch === "aarch64") {
      return { assetName: "export-Windows-ARM64.zip", arch: "arm64" };
    }
  }

  throw new Error(
    `Unsupported export runtime platform/arch: ${process.platform}/${arch}`
  );
}

const runtimeAsset = resolveRuntimeAsset();
const runtimeAssetName = runtimeAsset.assetName;
...
function writeInstalledVersion(version) {
  fs.writeFileSync(
    versionManifestPath,
    `${JSON.stringify({ version, asset: runtimeAssetName }, null, 2)}\n`,
    "utf8"
  );
}
...
function ensureCurrentConverterLink(converterPath) {
  ...
  const currentPath = path.join(
    targetPyDir,
    `convert-${process.platform}-current${extension}`
  );
...
function getConverterCandidates(baseDir = targetPyDir) {
  if (process.platform === "win32") {
    if (runtimeAsset.arch === "arm64") {
      return [
        path.join(baseDir, "convert-win32-arm64.exe"),
        path.join(baseDir, "convert.exe"),
      ];
    }
    return [
      path.join(baseDir, "convert-win32-x64.exe"),
      path.join(baseDir, "convert-win32-amd64.exe"),
      path.join(baseDir, "convert.exe"),
    ];
  }
...
function validateExistingRuntime(expectedVersion) {
  ...
  if (
    ...
    installedVersion.manifest.asset !== runtimeAssetName
  ) {
    return {
      ok: false,
      reason: [
        "Installed export runtime does not match package.json.",
        `Expected: ${expectedVersion} (${runtimeAssetName})`,
  ...
  if (!converterPath) {
    return {
      ok: false,
      reason: `No ${process.platform} converter binary under ${targetPyDir} or ${targetRoot}.`,
    };
  }
...
function unzipArchive(zipPath, destDir) {
  ...
  if (process.platform === "win32") {
    try {
      ...
      console.warn(
        "PowerShell Expand-Archive failed; trying unzip if it is installed.",
        err.message
      );
    }
  }
...
async function downloadAndInstallRuntime(tag) {
  const downloadUrl = `${exportRepoBase}/${tag}/${runtimeAssetName}`;

  ensureDir(cacheDir);
  const cacheKey = tag.replace(/[^a-zA-Z0-9._-]/g, "_");
  const zipPath = path.join(cacheDir, `${cacheKey}-${runtimeAssetName}`);
